...
Natural-origin qualification is disabled by default. The automation gateway accepts its issuer, control-plane source, environment, task/automation binding digests, schedule/configuration digests, and authority-epoch digest only from `ScheduledGovernance:NaturalOriginAuthority` server configuration. The corresponding `.env.example` values are intentionally empty: operators must provision verified non-secret identities and digests from the platform/control-plane trust setup. An absent, partial, malformed, or stale authority block remains `Unattested`; callers cannot supply or override it through MCP.
