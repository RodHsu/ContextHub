...
| `governance_batch_execute` | Executing one bounded administrator-only governance batch against a `knowledge_review` snapshot. A versioned cursor is bound to actor, tenant, project scope, execution policy, logical position, and expiry rather than one physical snapshot ID. After same-run re-review, the prior `nextCursor` safely maps to the new snapshot and the durable execution ledger excludes already terminal items. Each completed batch advances the next re-review generation; a repeated re-review without another completed batch replays the same immutable generation. Exact execution replay returns the original result and audit IDs even after newer generations exist. Scheduled mode is low-risk, proposal-first, archive-first, audited, protects DisplayName, and permanently disables hard-delete. Cursor failures return stable codes such as `CursorExpired`, `CursorActorMismatch`, `CursorScopeMismatch`, `CursorPolicyMismatch`, `ReReviewRequired`, and `ReplayPayloadMismatch`. |
