...
Its OAuth token must include `governance:scheduled`, which can be authorized only by a current active ContextHub tenant Owner or Admin. Authorization-code exchange and refresh rotation revalidate that role so demotion or deactivation fails closed; Member accounts remain usable on the general surface but cannot obtain automation authority. The connector exposes only `scheduled_governance_contract_get`, `scheduled_governance_review`, `scheduled_governance_execute`, and `scheduled_governance_run_get`. Call review with a fresh `governanceRunId`; obey the returned server decision. Call execute only for `ReversibleExecutionRequired`, replay the exact request after an unknown outcome, and use receipt read-back before retrying. Receipt read-back is always structured: `runExists=false` and `received=false` means the current connected ContextHub user has no receipt for that run, while a received `Running`, `Failed`, `Stopped`, or `Completed` receipt proves handler entry and supports outcome recovery. `NoOpConverged` is a valid successful run with zero mutation. `HumanDecisionOnly` and `CoverageIncomplete` stop unattended execution. Never switch to `/mcp-chat`, explicit ProjectIds, REST, DB, or admin endpoints to bypass a stop decision. The gateway may grant its internal application actor read-only access to the user's governance preferences because full review consumes them; this does not add `preferences:read` to the OAuth metadata or expose preference tools.

`scheduled_governance_run_get` also returns a read-only `reliability` projection derived from server receipts. It records the observed start time, intended execution slot, signed and absolute drift, cadence gaps, replay/manual-run exclusions, baseline resets, and the scheduler-versus-intended timezone mapping without adding another MCP tool or caller-controlled execution field. A run qualifies only when ContextHub can bind both a verified platform-signed `NaturalSchedule` attestation and an authenticated immutable control-plane execution audit to the exact actor, project scope, run, slot, initial review request, receipt, audience, contract, catalog, and runtime, and the server-owned receipt independently proves all mutation-safety requirements. Timing and nearest-slot calculation are observability and consistency checks only; missing, invalid, unavailable, or ambiguous provenance remains `Unattested`. ContextHub cannot change the host scheduler timezone; deployments that compensate one timezone for another must keep the intended and scheduler-local schedules explicit and restart the reliability window after a relevant contract, runtime, deployment, or schedule change.

Natural-origin qualification is disabled by default. The automation gateway accepts its issuer, control-plane source, environment, task/automation binding digests, schedule/configuration digests, and authority-epoch digest only from `ScheduledGovernance:NaturalOriginAuthority` server configuration. An append-only authority-epoch ledger provides an internal monotonic/non-reuse primitive, but it has no HTTP/MCP writer and does not authenticate a platform signature. A future trusted verifier must atomically bind configuration to the ledger's current committed generation before writing evidence; no trusted Production ingestion/verifier path is wired today, so configuration or schedule-aligned timing alone cannot qualify a run. The corresponding `.env.example` values are intentionally empty, and callers cannot supply or override authority through MCP.
...
| `conversation_insight_status` / `conversation_insight_retry` / `conversation_insight_skip` / `conversation_insight_set_disposition` | Inspecting and converging a staged insight. `set_disposition` records audited `Deferred`, `RequiresUserDecision`, or `HostBlocked` exceptions. An explicit retry requires both a fresh `governanceRunId` and a non-empty human reason; `HostBlocked` is never automatically retried. |
...
| `project_artifact_publish` | Publishing a project-scoped summary, snippet, or logical Managed File reference. File references require both ContextHub `FileId` and `FileVersionId`; provider locators are rejected. |
...
| `managed_file_register` | Registering an integrity-verified managed object as a logical Managed File. On `/mcp-chat` this is proposal-gated. |
| `managed_files_list` | Listing logical Managed Files for one authorized project without exposing storage or crypto identities. |
| `managed_files_search` | Searching logical Managed Files through provider-neutral file metadata and approved projections. |
...
The catalog summary is generated from the same canonical policy used by `/mcp-chat`, rather than a second configuration allowlist. Catalog `2026-09-27-v8` reports 83 backend `/mcp` tools and 83 ChatGPT App-facing tools, together with query/read, mutation, delete-capable, and proposal-gated counts. The general catalog includes all nine `agent_execution_*` worker tools and all seven Skills execution tools; `/mcp-automation` remains a separate exactly-four-tool surface. Release validation checks the host-compatible projection, including complete non-destructive annotations and structured schemas for AgentExecution plus the required read-only annotations for `governance_contract_get`, `governance_run_get`, and `governance_runs_list`. App-facing validation accepts JSON Schema outputs rooted at either `object` or `array`, while rejecting scalar roots that the connector cannot project as structured tool results.
