...
| `governance_run_get` / `governance_runs_list` | Reading the latest immutable cumulative receipt for one governance run or a bounded actor/project-scoped run list. Review-only, schema mismatch, host-blocked, batch execution, and internal retention terminal outcomes remain auditable without returning governed content. |
