...
- Exact pins are the execution authority. A later default-version change cannot alter an active execution. Revocation remains stronger than a pin and fails closed before read/materialization/invocation.
...
- Import rejects traversal, symlinks, duplicate paths, oversized bundles, unsupported executable placement, secret/private-key patterns, and invalid attestations. Executable self-tests require approval and are never reported as executed by the safe declarative validator.
