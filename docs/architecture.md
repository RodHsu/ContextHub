...
`governance_batch_execute` consumes that plan through the shared Application use cases used by REST, `/mcp`, and `/mcp-chat`. Its versioned cursor binds the actor/tenant, project scope, execution-policy hash, logical position, item key, version, and expiry. It deliberately does not bind continuation to one snapshot ID: after same-run re-review, the execution ledger suppresses already terminal items and maps the previously issued cursor to the new snapshot. Re-review snapshots are generation-based: a completed batch after the latest generation creates the next immutable generation, while retrying a re-review without another completed batch replays the same generation. Each execution remains bounded by mutation count, elapsed time, and cancellation. Successful, no-op, deferred, and user-decision items advance independently, while failed or unknown outcomes retain their cursor and audit evidence; dry-run results never enter the terminal ledger. Exact request replay returns the persisted result instead of repeating proposals or mutations, even after later snapshot generations exist. Scheduled mode requires an administrator and can only perform deterministic proposal-gated operations. It never changes DisplayName, completes ordinary business work, removes discussion history, promotes sensitive logs, or performs hard-delete, cross-project moves, shared-layer changes, protected-memory decisions, or semantic conflict resolution.
