...
Scheduled reliability evidence is persisted as an actor-scoped server projection and exposed only as nested read-only data on `scheduled_governance_run_get`; it does not add a fifth tool or broaden OAuth authority. Verified natural-origin records are stored as minimal append-only digests rather than raw attestation, audit, prompt, token, or user payloads. A run qualifies only when a platform-signed `NaturalSchedule` attestation and an authenticated immutable control-plane execution audit independently bind the exact actor, project scope, run, slot, initial review request, receipt, audience, contract, catalog, and runtime, while the immutable server receipt separately proves coverage, decision obedience, mutation safety, and executor boundaries. Missing, invalid, unavailable, replay-conflicting, or ambiguous evidence fails closed as `Unattested`; a later authoritative audit may be reflected by read-only re-evaluation without rewriting the receipt or reliability ledger. The calculator canonicalizes exact replay, ignores manual and non-scheduled runs, detects missing, duplicate, and out-of-order schedule slots, binds runs to contract/catalog/runtime identity, and resets continuity on relevant deployment or configuration changes. A signed `ExpectedAtUtc` is the only slot authority for qualification. The nearest intended slot resolved from `IntendedTimeZoneId` and `IntendedLocalRunTimes` remains useful for drift observability but cannot establish provenance. `SchedulerTimeZoneId` and `SchedulerLocalRunTimes` are persisted audit metadata describing host compensation, not an alternate authority; tests require the configured Asia/Tokyo compensation to map to the same UTC slots as the intended Asia/Taipei cadence. ContextHub cannot change the host scheduler timezone, and schedule-aligned timing alone is never accepted as natural-run provenance.
