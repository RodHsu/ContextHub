...
`governance_batch_execute` consumes that snapshot through the shared Application use cases used by REST, `/mcp`, and `/mcp-chat`. A persisted batch run owns an immutable plan and cursor; each execution is bounded by mutation count, elapsed time, and cancellation. Successful, no-op, deferred, and user-decision items advance independently, while failed or unknown outcomes retain their cursor and audit evidence. Exact request replay returns the persisted result instead of repeating proposals or mutations. Scheduled mode requires an administrator and can only perform low-risk proposal-gated update, archive, reconciliation, or reindex operations. It never performs hard-delete, cross-project moves, shared-layer promotion, protected-memory decisions, or semantic conflict resolution. New data created after a snapshot is discovered by same-run re-review and processed in a new snapshot phase.
