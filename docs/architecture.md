...
Every review, execution, terminal contract/host-blocked outcome, and internal retention batch appends an immutable `governance_run_receipts` event. `governance_run_get` reads the latest cumulative receipt for one run and `governance_runs_list` returns a bounded actor/project-scoped list. Receipts separate candidate, execution-actionable, and governed-exception counters, and preserve snapshot, convergence, stop reason, contract, schema, audit, and retention counters without storing governed content.
