...
Scheduled reliability evidence is persisted as an actor-scoped server projection and exposed only as nested read-only data on `scheduled_governance_run_get`; it does not add a fifth tool or broaden OAuth authority. The calculator canonicalizes exact replay, ignores manual and non-scheduled runs, detects missing, duplicate, and out-of-order schedule slots, binds runs to contract/catalog/runtime identity, and resets continuity on relevant deployment or configuration changes. It stores expected-versus-observed drift and both the scheduler-local and intended timezone schedules. The intended schedule is the reliability authority: a platform-provided `ExpectedAtUtc` wins when present, otherwise ContextHub resolves the nearest slot from `IntendedTimeZoneId` and `IntendedLocalRunTimes`. `SchedulerTimeZoneId` and `SchedulerLocalRunTimes` are persisted audit metadata describing host compensation, not an alternate authority; tests require the configured Asia/Tokyo compensation to map to the same UTC slots as the intended Asia/Taipei cadence. ContextHub cannot change the host scheduler timezone, and timing alone is never accepted as natural-run provenance: without platform-signed attestation, the origin status remains `Unattested` and the run is non-qualifying even when its timestamp matches the expected slot.
