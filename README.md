...
This surface publishes exactly four tools: `scheduled_governance_contract_get`, `scheduled_governance_review`, `scheduled_governance_execute`, and `scheduled_governance_run_get`. Review resolves the actor's full authorized durable scope server-side and returns an immutable snapshot, count invariant, and one fixed decision (`NoOpConverged`, `ReversibleExecutionRequired`, `HumanDecisionOnly`, or `CoverageIncomplete`). Its internal application actor may read the connected user's governance preferences, but the OAuth resource remains limited to `governance:scheduled`; no preference tool or preference-write capability is exposed. Only an active ContextHub tenant Owner or Admin may authorize, exchange, or refresh this scheduled-governance scope; Member accounts remain valid for the general App but fail closed before an automation token is issued. Execute accepts no ProjectIds, action list, risk policy, hard-delete, matured-delete, dry-run, or execution-mode controls; it can run only the server's fixed low-risk reversible policy with snapshot/cursor binding and authorization revalidation. Irreversible matured retention remains exclusively owned by the internal Worker. General `/mcp` and `/mcp-chat` capabilities are unchanged.

`scheduled_governance_run_get` always returns structured recovery state. `runExists=false` and `received=false` proves that the current connected ContextHub user has no receipt for that run; `received=true` with `status=Running`, `Failed`, `Stopped`, or `Completed` proves that dispatch reached the governance handler. Failure receipts expose only a bounded failure phase and class, never raw tool arguments or credentials.
...
- [Platform Wave 4B MFA and assurance](docs/platform-wave-4b-mfa.md)
- [Platform Wave 7A logical Managed File breaking cutover](docs/platform-wave-7a-breaking-cutover.md)
