...
It is separate from `/mcp` and is intended for OAuth/OIDC-authenticated clients, project allowlists, rate limits, audit trails, restricted tool discovery, proposal-gated durable knowledge writes, and direct project work-item lifecycle operations. Scheduled governance uses `knowledge_review` with a stable `governanceRunId`, then calls the server-side `governance_batch_execute` tool with the returned snapshot token and continuation cursor. The unified compact plan covers Project Information, hierarchy, Project/Shared knowledge, preferences, artifacts, discussions, work items, conversation insights, Suggested Actions, proposals, and bounded server-side log partitions. The executor persists an actor-scoped plan, applies only bounded low-risk operations, performs resource read-back, returns per-item audit references, and continues a backlog across later scheduled runs without treating a page-size threshold as a stop condition. Its versioned cursor binds actor, tenant, project scope, execution policy, logical position, item key, and expiry; after same-run re-review it maps safely to the new snapshot and excludes already terminal ledger items. Scheduled execution is administrator-only, proposal-first, archive-first, protects UI-managed DisplayName, and always rejects hard-delete. Semantic conflicts, protected data, cross-project moves, shared-layer changes, sensitive logs, ambiguous hierarchy, and irreversible deletion remain explicit user decisions. Callers must review every candidate page, execute bounded batches, follow `nextCursor`, and perform same-run re-review. Normal unfinished business Work Items are reported separately and never block knowledge/data convergence. Completion requires every surface `coverageComplete=true`, every `hasMore=false`, and `governanceActionableCount=0`; governed exceptions return `ConvergedWithExceptions`. The full Codex/agent MCP endpoint remains `/mcp`.
